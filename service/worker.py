...
class Worker:
    def __init__(self, llm: ChatClient, fs: FeatureStore, config_path:str):
        ...
        if llm_config['enable_local']:
            self.context_max_length = llm_config['server']['local_internlm_max_text_length']
        elif llm_config['enable_remote']:
            self.context_max_length = llm_config['server']['remote_kimi_max_text_length']
        else:
...
“question:{} answer:{}”'''
    ...
    def generate(self, query, history, groupname):
        ...
        if not self.single_judge(prompt=self.SCORING_QUESTION_TEMPLTE.format(query), tracker=tracker, throttle=7, default=3):
        ...
        if self.single_judge(self.SCORING_RELAVANCE_TEMPATE.format(query, db_context_part), tracker=tracker, throttle=5, default=10):
            prompt, history = llm.build_prompt(instruction=query, context=db_context, history_pair=history, template=self.GENERATE_TEMPLATE)
            ...
            try:
                ...
                articles = web_search.get_with_cache(query=web_keywords, max_article=2)
                ...
                for article in articles:
                    if article is not None and len(article) > 0:
                        ...
                        if self.single_judge(self.SECURITY_TEMAPLTE.format(article), tracker=tracker, throttle=3, default=0):
                            tracker.log('跳过不安全的内容', article)
                            continue

                        if self.single_judge(self.SCORING_RELAVANCE_TEMPATE.format(query, article), tracker=tracker, throttle=6, default=10):
                            web_context += '\n\n'
                            web_context += article
                ...
                if len(web_context) > 0:
                    prompt, history = llm.build_prompt(instruction=query, context=web_context, history_pair=history, template=self.GENERATE_TEMPLATE)
        ...
        if response is not None and len(response) > 0:
            ...
            if self.single_judge(prompt=prompt, tracker=tracker, throttle=8, default=0):
        ...
        if self.config['worker']['enable_sg_search']:
            if reborn_code == ErrorCode.BAD_ANSWER or reborn_code == ErrorCode.NO_SEARCH_RESULT:
                ...
                sg_context = sg.search(llm=llm, question=query, groupname=groupname)
                if sg_context != None and len(sg_context) > 0:
                    prompt, history = llm.build_prompt(instruction=query, context=sg_context, history_pair=history, template=self.GENERATE_TEMPLATE)

                    response = llm.generate_response(prompt=prompt, history=history, remote=True)
                    tracker.log('source graph', [sg_context, response]) 

                    prompt = self.PERPLESITY_TEMPLATE.format(query, response)
                    if self.single_judge(prompt=prompt, tracker=tracker, throttle=9, default=0):
                        return ErrorCode.BAD_ANSWER, response

        if response is not None and len(response) >= 600:
            # 回复内容太长，总结一下
            response = llm.generate_response(prompt=self.SUMMARIZE_TEMPLATE.format(response))

        if len(response) > 0 and self.single_judge(self.SECURITY_TEMAPLTE.format(response), tracker=tracker, throttle=3, default=0):
