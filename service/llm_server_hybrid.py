...
class HybridLLMServer(object):
    ...
    def __init__(self, llm_config: dict, device: str = "cuda", retry=3) -> None:
        self.device = device
        self.retry = retry
        self.llm_config = llm_config
        self.local_max_length = llm_config['server_local_internlm_max_text_length']
        self.remote_max_length = llm_config['server_remote_kimi_max_text_length']
    ...
    def generate_response(self, prompt, history=[], remote=False):
        ...
        if not llm_config['server_remote_kimi_enable'] and remote:
            remote = False
            logger.error('llm.server_remote_kimi_enable off, auto set remote=False')
...
if __name__ == '__main__':
    ...
    async def inference(request):
        ...
        text = server.generate_response(prompt=prompt, history=history, remote=remote)
