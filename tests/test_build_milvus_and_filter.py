...
from pymilvus import (
    ...
    Collection, AnnSearchRequest, RRFRanker, WeightedRanker, connections, db
)
...
from pymilvus.model.hybrid import BGEM3EmbeddingFunction
ef = BGEM3EmbeddingFunction(model_name='/data2/khj/bge-m3', use_fp16=False, device="cuda", batch_size=64)
...
def init_milvus(col_name: str, max_length_bytes: int):
    ...
    fields = [
        ...
        FieldSchema(name="text", dtype=DataType.VARCHAR, max_length=max_length_bytes),
    ...
    for sparse_ratio in sparse_ratios:
        for text_label in tqdm(text_labels):
            ...
            sparse_req = AnnSearchRequest(query_embeddings["sparse"],
                                        "sparse_vector", {"metric_type": "IP"}, limit=k)
            dense_req = AnnSearchRequest(query_embeddings["dense"],
                                        "dense_vector", {"metric_type": "IP"}, limit=k)

            # Search topK docs based on dense and sparse vectors and rerank with RRF.
            res = col.hybrid_search([sparse_req, dense_req], rerank=WeightedRanker(sparse_ratio, 1.0-sparse_ratio),
