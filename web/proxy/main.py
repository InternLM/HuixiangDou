...
class CacheRetriever:
    ...
    def get(self, fs_id: str):
        ...
        retriever = Retriever(embeddings=self.embeddings, reranker=self.reranker, work_dir=workdir, reject_throttle=reject_throttle)
        self.cache[fs_id] = {
            'retriever' = retriever,
            'time' = time.time()
        }
...
def callback_chat_state(feature_store_id: str, query_id:str, code:int, status: str, text:str, ref:list):
    que = Queue(name='ChatResponse',host=redis_host(), port=redis_port(), charset='utf-8', decode_responses=True)
...
def format_history(history):
    """format [{sender, content}] to [(user, bot)] style."""
    ret = []
    last_id = -1
...
def chat_with_featue_store(cache: CacheRetriever, payload: types.SimpleNamespace):
    # "payload": {
    ...
    chat_state = partial(callback_chat_state, feature_store_id=fs_id, query_id=query_id)
    retriever, error = cache.get(fs_id=fs_id)

    if error is not None:
        chat_state(code=ErrorCode.INTERNAL_ERROR, status=ErrorCode.INTERNAL_ERROR.describe(), text='', ref=[])
        return
    ...
    error, response, references = worker.generate(prompt, history)
    if error == ErrorCode.SUCCESS:
        chat_state(code=ErrorCode.INTERNAL_ERROR, status=ErrorCode.INTERNAL_ERROR.describe(), text='', ref=[])
        return
    chat_state(code=ErrorCode.SUCCESS, status=ErrorCode.SUCCESS.describe(), text=response, ref=references)
