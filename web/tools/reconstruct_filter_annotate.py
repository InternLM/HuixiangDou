...
话题跟踪：群聊中的话题可能会快速变化，跟踪当前的讨论主题对于正确理解意图至关重要。 歧义处理：群聊中的信息往往更加随意和含糊，需要能够处理语言上的歧义。
情感分析：理解发言者的情感状态可以帮助更准确地把握其意图。 礼貌和文化差异：群聊中可能包含来自不同文化背景的人，理解和尊重这些差异对于正确消解意图很重要。
指令识别：在群聊中，用户可能会发出指令或请求，识别这些指令并作出适当的响应是必要的。
信息过滤：群聊中可能包含大量的信息，能够过滤掉无关信息，专注于相关的对话片段。 隐私和安全：在处理群聊数据时，保护用户隐私和数据安全是非常重要的。
响应时间：在群聊中，及时响应可以提升用户体验，但也要注意不要过早地打断正在进行的对话。
多任务处理：群聊可能同时涉及多个话题或任务，能够同时处理多个意图是有帮助的。
错误处理：当理解出现偏差时，能够优雅地处理错误并从用户那里获得反馈以改进是重要的。
用户引导：在必要时，引导用户提供更清晰的信息或指令，以帮助更好地理解其意图。 适应性：群聊的动态性要求系统能够适应不断变化的对话模式和用户行为。
...
def metric(dt_key='cr_need'):
    gts = []
    dts = []

    with open('groups/gt.jsonl') as gt:
        for line in gt:
